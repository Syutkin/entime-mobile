...
class UpdateState with _$UpdateState {
  const factory UpdateState.initial({String? changelog}) = _UpdateInitial;

  // Есть обновление, вызываем попап с вопросом обновлять или нет
  const factory UpdateState.updateAvailable({required String version}) = _UpdateAvailable;
  ...
  const factory UpdateState.downloading({required int bytes, required int total}) = _UpdateDownloading;

  const factory UpdateState.downloadError({required String error}) = _UpdateDownloadError;
