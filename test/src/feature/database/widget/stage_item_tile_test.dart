...
void main() {
  ...
  group(
    ...
    () {
      ...
      patrolWidgetTest('Tap trailing icon', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget());
      ...
      patrolWidgetTest('Tap delete, than cancel', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget());
        await $(PopupMenuButton<void>).tap();
        ...
        verifyNever(
          () => databaseBloc.add(DatabaseEvent.deleteStage(stage.id)),
        );
      });

      patrolWidgetTest('Tap delete, than ok', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget());
        await $(PopupMenuButton<void>).tap();
        await $(Localization.current.I18nCore_delete).tap();
        await $(#okButton).tap();
        verify(() => databaseBloc.add(DatabaseEvent.deleteStage(stage.id)))
            .called(1);
      });

      patrolWidgetTest('Tap on widget', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget());
        await $(StageItemTile).tap();
        verify(() => databaseBloc.add(DatabaseEvent.selectStage(stage)))
            .called(1);
        verify(() => countdownBloc.add(CountdownEvent.start(stageId: stage.id)))
            .called(1);
        // pop
