  ...
  group(
    ...
    () {
      group('General widgets', () {
        ...
        patrolWidgetTest(
          ...
          (PatrolTester $) async {
            await $.pumpWidgetAndSettle(testWidget());
            expect(
              $(Localization.current.I18nStart_sliverNumber),
              findsOneWidget,
            );
            expect(
              $(Localization.current.I18nStart_sliverStart),
              findsOneWidget,
            );
            expect(
      ...
      group('Countdown floating widget', () {
        ...
        patrolWidgetTest(
          'Show countdown if enabled in settings',
          (PatrolTester $) async {
            when(
              () => countdownBloc.state,
        ...
        patrolWidgetTest(
          ...
          (PatrolTester $) async {
            ...
            await $.pumpWidgetAndSettle(testWidget());
            expect(
              ($(Container).evaluate().single.widget as Container)
                  .constraints
                  ?.maxWidth,
              75,
            );
        ...
        patrolWidgetTest(
          'Drag countdown and update settings with new place',
          (PatrolTester $) async {
            settings = settings.copyWith(countdown: true);
            const text = 'countdown text';
            when(
      ...
      group('FAB button', () {
        ...
        patrolWidgetTest(
          'Hide when disabled at settings',
          (PatrolTester $) async {
        ...
        patrolWidgetTest(
          'Tap FAB and do nothing when stage not selected',
          (PatrolTester $) async {
            when(() => databaseBloc.state).thenReturn(
              const DatabaseState(
                races: [],
      ...
      group('Start list', () {
        ...
        patrolWidgetTest(
          ...
          (PatrolTester $) async {
            ...
            expect(
              $(PopupMenuItem<StartPopupMenu>),
              findsNWidgets(2),
            );
            expect($(Localization.current.I18nCore_edit), findsOneWidget);
            expect(
              $(Localization.current.I18nStart_shiftStartsTime),
              findsOneWidget,
            );
          },
        );
        ...
        patrolWidgetTest(
          ...
          (PatrolTester $) async {
            ...
            expect($(StartItemTile), findsNWidgets(3));
            await $.tester.drag(
              $(StartItemTile).containing('11:00:00'),
              const Offset(500, 0),
            );
            await $.pumpAndSettle();
            verify(
              () => databaseBloc.add(
                const DatabaseEvent.setStatusForStartId(
                  startId: 1,
...
final riders = <Rider>[
  const Rider(id: 1, name: 'name', isDeleted: false),
];
