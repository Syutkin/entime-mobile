...
void main() {
  ...
  group('CountdownBloc tests', () {
    test('Initial state when stage not selected', () async {
    ...
    blocTest<CountdownBloc, CountdownState>(
      ...
      setUp: () {
        when(() => countdownAtStart.ticks).thenAnswer((_) => BehaviorSubject<Tick>.seeded(tick));
      },
      build: () => CountdownBloc(audioController: audioController, countdown: countdownAtStart, stageId: stageId),
      expect: () => [CountdownState.working(tick: tick)],
    );
    ...
    blocTest<CountdownBloc, CountdownState>(
      ...
      setUp: () {
        for (var i = 0; i < 5; i++) {
          ticks.add(Tick(second: i, text: text, nextStartTime: DateTime.now(), number: i));
        }
        when(
          () => countdownAtStart.ticks,
        ).thenAnswer((_) => BehaviorSubject<Tick>()..addStream(Stream.fromIterable(ticks)));
    ...
    blocTest<CountdownBloc, CountdownState>(
      ...
      build: () => CountdownBloc(audioController: audioController, countdown: countdownAtStart, stageId: -1),
      verify: (_) {
