...
void main() {
  ...
  Widget testWidget() {
    return MaterialApp(
      ...
      home: Material(child: BlocProvider.value(value: settingsBloc, child: const SettingsScreen())),
    );
  }
  ...
  group('Settings screen tests', () {
    setUp(() async {
      ...
      settingsBloc = SettingsBloc(sharedPrefsSettingsProvider);
    });
    ...
    patrolWidgetTest('Check themes section', (PatrolTester $) async {
      ...
      for (final colorSeed in ColorSeed.values) {
        expect(await $(SettingsSection).containing(Localization.current.I18nSettings_themes).$(SettingsTile).$(colorSeed.name).scrollTo(), findsOneWidget);
      }
    });
