  ...
  group(
    ...
    () {
      patrolWidgetTest('Initial build', (
        ...
        expect(
          ($(ListTile).evaluate().single.widget as ListTile).subtitle,
          null,
        );
        expect(
      ...
      patrolWidgetTest('Trail with additional info', (
        PatrolTester $,
      ) async {
        trail = TrailInfo(
          row: row,
        ...
        expect(
          $('${DateFormat.yMMMd().format(now)}, ${filesize(size)}'),
          findsOneWidget,
        );
        expect(
          ($(Icon).at(0).evaluate().single.widget as Icon).icon,
      ...
      patrolWidgetTest('Tap PopupMenuButton', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget(trail));
      ...
      patrolWidgetTest('Tap delete then ok', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget(trail));
        await $(PopupMenuButton<void>).tap();
        ...
        await $(#okButton).tap();
        verify(() => trailsBloc.add(TrailsEvent.deleteTrail(trail.id)))
            .called(1);
      });

      patrolWidgetTest('Tap delete then cancel', (
        PatrolTester $,
      ) async {
        await $.pumpWidgetAndSettle(testWidget(trail));
        await $(PopupMenuButton<void>).tap();
      ...
      patrolWidgetTest('Tap PopupMenuButton on trail with track', (
        PatrolTester $,
      ) async {
        trail = TrailInfo(
          row: row,
        ...
        verify(() => databaseBloc.add(DatabaseEvent.shareTrack(trail: trail)))
            .called(1);
      });

      patrolWidgetTest('Tap share', (
        PatrolTester $,
      ) async {
        trail = TrailInfo(
          row: row,
