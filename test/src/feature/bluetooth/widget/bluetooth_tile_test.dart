...
void main() {
  ...
  Widget testWidget() {
    return MaterialApp(
      ...
      home: Material(
        child: BlocProvider.value(value: btBloc, child: const BluetoothTile()),
      ),
    );
  }
  ...
  group('BluetoothTile tests', () {
    ...
    patrolWidgetTest('notAvailable state', (PatrolTester $) async {
      when(
        () => btBloc.state,
      ).thenReturn(const BluetoothBlocState.notAvailable());
      await $.pumpWidgetAndSettle(testWidget());
      expect(($(ListTile).evaluate().single.widget as ListTile).enabled, false);
      expect(($(ListTile).evaluate().single.widget as ListTile).trailing, null);
      expect(
    ...
    patrolWidgetTest('notEnabled state', (PatrolTester $) async {
      when(
        () => btBloc.state,
      ).thenReturn(const BluetoothBlocState.notEnabled());
      await $.pumpWidgetAndSettle(testWidget());
      expect(($(ListTile).evaluate().single.widget as ListTile).enabled, false);
      expect(($(ListTile).evaluate().single.widget as ListTile).trailing, null);
      expect($(Localization.current.I18nInit_bluetoothModule), findsOneWidget);
    ...
    patrolWidgetTest('connecting state w/o bluetoothDevice at bloc', (
      PatrolTester $,
    ) async {
      when(
        () => btBloc.state,
      ).thenReturn(const BluetoothBlocState.connecting());
      await $.pumpWidgetAndSettle(testWidget());
      ...
      await $(ListTile).tap();
      expect(
        $(Localization.current.I18nBluetooth_selectDevice),
        findsOneWidget,
      );
    });

    patrolWidgetTest('connecting state with bluetoothDevice at bloc', (
      PatrolTester $,
    ) async {
      when(
        () => btBloc.state,
      ).thenReturn(const BluetoothBlocState.connecting());
      when(() => btBloc.bluetoothDevice).thenReturn(
        const BluetoothDevice(address: '', name: 'BluetoothDeviceName'),
      );
      await $.pumpWidgetAndSettle(testWidget());
      expect(($(ListTile).evaluate().single.widget as ListTile).enabled, true);
      expect(($(ListTile).evaluate().single.widget as ListTile).trailing, null);
    ...
    patrolWidgetTest('Press log button at connected state', (
      PatrolTester $,
    ) async {
      when(() => btBloc.state).thenReturn(const BluetoothBlocState.connected());
      when(() => btBloc.bluetoothDevice).thenReturn(
    ...
    patrolWidgetTest('Press module settings button at connected state', (
      PatrolTester $,
    ) async {
      when(() => btBloc.state).thenReturn(const BluetoothBlocState.connected());
      when(() => btBloc.bluetoothDevice).thenReturn(
    ...
    patrolWidgetTest('disconnecting state', (PatrolTester $) async {
      when(
        () => btBloc.state,
      ).thenReturn(const BluetoothBlocState.disconnecting());
      when(() => btBloc.bluetoothDevice).thenReturn(
        const BluetoothDevice(address: '', name: 'BluetoothDeviceName'),
      );
      await $.pumpWidgetAndSettle(testWidget());
